...
count_df <- select(count_df,-c(Libraries)) + 1 
count_df$Libraries <- lib_column

if(dataset=='Okamura'){
  norm_count_df <- select(count_df, -c(spikeinset01,`dmel-all-chromosome-r6.22`,Libraries)) * 1000 / count_df$spikeinset01
}else{
  norm_count_df <- select(count_df, -c(spikeinset01,`dmel-all-chromosome-r6.22`,Libraries)) * 1000000 / count_df$`dmel-all-chromosome-r6.22`
}
...
patch_normcount_plot2 <- ggplot(melted_group_normcount_df, aes(x= Condition, y=mean_normcounts)) +
